...
function MenuCard({ item, itemTranslation }) {
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow">
      <img
        ...
        onError={(e) => {
          e.target.src = `https://placehold.co/400x300/F5E6C8/8B6914?text=${encodeURIComponent(itemTranslation.name)}`
        }}
      />
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-lg font-semibold text-charcoal font-display">{itemTranslation.name}</h3>
          <span className="text-deep-red font-bold">${item.price}</span>
        </div>
        <p className="text-muted text-sm leading-relaxed">{itemTranslation.description}</p>
      </div>
    </div>
  )
}

function MenuSection({ section, sectionTranslation, itemsTranslation }) {
  return (
    <section className="mb-16">
      <h2 className="text-2xl md:text-3xl font-bold text-charcoal font-display text-center mb-2">{sectionTranslation.title}</h2>
      <p className="text-muted text-center mb-2">{sectionTranslation.subtitle}</p>
      <div className="w-16 h-0.5 bg-gold mx-auto mb-10"></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {section.items.map((item) => (
          <MenuCard key={item.key} item={item} itemTranslation={itemsTranslation[item.key]} />
        ))}
      </div>
    </section>
  )
    ...
    <div>
      <div className="relative h-40 md:h-64 bg-cover bg-center flex items-center justify-center"
        ...
        <div className="relative text-center text-white z-10">
          <h1 className="text-3xl md:text-5xl font-bold font-display mb-2">{t.menu.pageTitle}</h1>
          <p className="text-lg text-white/80">{t.menu.pageSubtitle}</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 py-8 md:px-8 md:py-16">
        <MenuSection section={menuData.signature} sectionTranslation={t.menu.signature} itemsTranslation={t.menu.items} />
        <MenuSection section={menuData.seasonal} sectionTranslation={t.menu.seasonal} itemsTranslation={t.menu.items} />
        <MenuSection section={menuData.sides} sectionTranslation={t.menu.sides} itemsTranslation={t.menu.items} />
      </div>
    </div>
  )
}
